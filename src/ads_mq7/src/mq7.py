...
class MQ7():
    ...
    def __init__(self, Ro=10):
        ...
        # create an analog input channel on pin 0 for MQ7
        self.chan_MQ7 = AnalogIn(ads, ADS.P0)
        
        self.H2Curve = [2.3,0.10,-0.20]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent"
                                            # to the original curve. 
                                            # data format:{ x, y, slope}; point1: (lg50, 0.10), point2: (lg4000, -0.28) 
        self.COCurve = [2.3,0.22,-0.14]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg50, 0.22), point2: (lg4000, -0.04)
        self.LPGCurve =[2.3,0.95,-0.13]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg50, 0.95), point2: (lg4000,  0.70)
        self.CH4Curve =[2.3,1.17,-0.12]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg50, 1.17), point2: (lg4000,  0.95)
        self.AlcoholCurve =[2.3,1.23,-0.06] # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg50, 1.23), point2: (lg4000,  1.11)
                
        print("Calibrating MQ-7...")
        self.Ro = self.MQ7_Calibration()
    ...
    #          27, which differs slightly between different sensors.
    ############################################################################ 
    def MQ7_Calibration(self):
        val = 0.0
        for i in range(self.CALIBRATION_SAMPLE_TIMES):          # take multiple samples
            val += self.MQResistanceCalculation(self.chan_MQ7.value)
    ...
    def MQRead(self):
        ...
        for i in range(self.READ_SAMPLE_TIMES):
            raw_value += self.chan_MQ7.value
            rs += self.MQResistanceCalculation(self.chan_MQ7.value)
